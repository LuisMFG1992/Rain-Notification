...
const getDataOnLoad = async (lat, long) => {
  try {
    if (navigator.geolocation) {
      ...
      cardCityName.innerText = `City: ${cityUpperCase}`;
      ...
      cardDescription.innerText = `Description: ${statusToUpperCase}`;

      let temp = Math.round(respuestaJSON2.main.temp);
      cardCityTemp.innerText = `Temp: ${temp}°C`;
    } else {
      alert("ERROR EN TRY.");
    }
  } catch (err) {
...
const getData = async (city) => {
  try {
    ...
    if (respuesta.ok) {
      ...
      alert(
        "The name of the city or country you are searching is invalid. Please try again."
      );
    }
  } catch (err) {
    console.log("ERROR", err);
